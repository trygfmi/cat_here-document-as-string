...
<p>このリポジトリは、catコマンドを実行してヒアドキュメントに記述された変数がファイルに文字列として出力されることを体験できます</p>
</div>
...
<div class="wp-block-group">
<p>上記のコマンドをインストール済みの方は、以下のコマンドを実行してリポジトリからダウンロード後、ディレクトリを移動し、catコマンドを実行してファイルに出力された文字列を表示してみてください</p>
...
<pre class="wp-block-code has-background" style="background-color:#ffeeee"><code>$hello</code></pre>
</details>
</div>
...
<pre class="wp-block-code has-24292-eff-color has-text-color has-background has-1-125-rem-font-size" style="background-color:#ffeeee"><code>$hello</code></pre>
</details>
</div>
</details>
...
<div class="wp-block-group">
<p>cat &lt;&lt; 'EOF' > test.txtのEOFの前後に'(シングルクオーテーション)を記述することで次の行からEOFまでの変数が文字列として扱われるようになります</p>
</div>
